...
time_node_reassignment <- as.POSIXct("2026-04-16 04:40:00", tz = "UTC")
...
df_trend_projection <- data.frame(hours_elapsed = c(hours_node_reassignment, max(df_exp3$hours_elapsed) + 3))
...
proj_text_x <- proj_label_x - 1.5
proj_text_y <- proj_label_y - 13000

max_hours <- max(ceiling(max(df_crawl_progress$hours_elapsed)), 19)
...
ggplot(df_crawl_progress, aes(x = hours_elapsed, y = sites_completed, color = experiment_id)) +
  ...
  annotate("text",
    ...
    hjust = 0.5, vjust = 0.5, size = 5, color = "#10b981", fontface = "italic"
  ) +
  annotate("segment",
    x = proj_text_x + 0.2, y = proj_text_y + 3000,
    xend = proj_text_x - 0.4, yend = proj_label_y - 6000,
  ...
  annotate("text",
    ...
    hjust = -0.05, vjust = -0.5, size = 5, color = "grey30"
  ) +
  annotate("text",
    x = hours_node_reassignment, y = Inf, label = "Node reassignment\n(50 -> 100 for Chromium)",
    vjust = 4.6, hjust = 1.1, size = 5, color = "grey30"
  ) +
  annotate("segment",
  ...
  scale_x_continuous(breaks = hour_breaks, labels = hour_labels, limits = c(0, max_hours)) +
...
ggsave("plots/crawl_progress.pdf", width = 12, height = 5)
